...
public class Main {
    ...
    public static void main(String[] args) throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        ...
        /*
         * YCM: The high level flow
         * block handler -> init with blockchain which is init with genesis block
         * 1st block ->
         *   1st tx -> init, add input/output, sign, add to block
         * block hanlder process 1stBlock
         * the block loop continue ...
         */
        ...
        //YCM: my own testing
        Block block6 = new Block(block1.getHash(), pk_alice.getPublic());
        Tx tx6 = new Tx();
        tx6.addInput(genesis.getCoinbase().getHash(), 0);  //25
        tx6.addOutput(15, pk_scrooge.getPublic());
        tx6.addOutput(10, pk_bob.getPublic());
        tx6.signTx(pk_scrooge.getPrivate(), 0);
        block6.addTransaction(tx6);
        block6.finalize();
